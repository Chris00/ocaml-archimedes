module type T = sig
  type style =
    ...
    | Separated
        (** The regions are separated by a gap *)
    | HighlightFlat
        (** One of the regions is separated by a gap, and so highlighted
            toward the others *)
    | Relief
        (** A simple 3D pie *)
  ...
  type colorscheme =
    ...
    | None
        (** No colors at all, the Overpie or Outer keyscheme should then
            be used *)
  ...
  type keyplacement =
    | Rectangle
        (** A rectangle containing the color followed by the label of each
            data *)
    | OverPie
        (** The labels are drawn directly over the data *)
    | Outer
        (** The labels are drawn around the pie, next to the data they
            point out *)
  ...
  type keylabels =
    ...
    | WithProcents
        (** The name followed by the procent among all data between
            parentheses *)
    | CustomLabels of (string -> float -> float -> string)
        (** A custom label made of the name, the value and the
            percentage *)
