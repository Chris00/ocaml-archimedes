...
let point_string x y = sprintf "(%.16f, %.16f)" x y
...
struct
  ...
  let restore t =
    ...
    try
      ...
      write t "\\end{scope}";
    with Stack.Empty ->
      printf "archimedes_tikz : warning - restore without saving\n%!"
  ...
  let make ~options width height =
    let make_opts =
      let device_rectangle = sprintf "(0,0) rectangle (%g,%g)" width height
      in function
    ...
    match options with
      name::opts ->
        ...
        output_string writer "%%% Generated by Archimedes OCaml module\n";
  ...
  let set_color t c =
    ...
    write t (sprintf "\\definecolor{archimedes_tikz %i}{rgb}{%.2f, %.2f, %.2f}"
  ...
  let select_font_face t slant weight family =
    ...
    in
    write t (sprintf "\\usefont{T1}{%s}{%s}{%s}" family series shape)
