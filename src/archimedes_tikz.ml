...
struct
 ...
 (* let print_ctm st =
  ...
  let make ~options width height =
    match options with
      [name] ->
        ...
        let w,h = string_of_float w, string_of_float h in
  ...
  let set_color t c =
    ...
    write t (Printf.sprintf "\definecolor{archimedes_tikz %i}{%f, %f, %f}"
               st.color_number r g b);
    st.color <- "color=archimedes_tikz "^(string_of_int st.color_number);
    st.opacity <- "opacity="^(string_of_float a);
    st.color_number <- st.color_number + 1;

  let set_line_width t w =
    let st = get_state t in
    st.line_width <- sprintf "line width=%f" w;

  let get_line_width t =
    let s = (get_state t).line_width in
    let len = String.length s in
    let s' = String.sub 11 (len - 13) in (*removes 'line width=' and 'cm'*)
    float_of_string s'
  ...
  let move_to t ~x ~y =
    let st = get_state t in
    let x,y = B.Matrix.transform_point x y in
  ...
  let curve_to t ~x1 ~y1 ~x2 ~y2 ~x3 ~y3 =
    ...
    if not st.curr_pt then t.curr_path <- t.curr_path^" "^(point x1' y1');
    t.curr_path <- t.curr_path^" ..controls "^(point x1' y1')^
      " and "^(point x2' y2')^".. "^(point x3' y3');
  ...
  let rectangle t ~x ~y ~w ~h =
    ...
    t.curr_path <-t.curr_path^" "^(point x y)^" rectangle "^
      (point (x'+.w') (y'+.h'));
    (* Update the current point and extents *)
    st.path_extents <-
  ...
  let clip_rectangle t ~x ~y ~w ~h =
    write t ("\clip "^(point x y)^" rectangle "^(point (x+.w) (y+.h)";")
  ...
  let show_text t ~rotate ~x ~y pos txt =
    ...
    let pos = match pos with
      ...
      | B.RB -> "north west"
    in
    write t (Printf.sprintf "\node [anchor=%s,rotate=%f] at %s {%s}"
               pos angle (point x' y') txt
end
