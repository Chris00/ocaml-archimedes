 ...
 ***********************************************************************)
  ...
  let stroke_preserve ?path ?(do_fit=true) vp coord_name =
  ...
  let stroke ?path ?do_fit vp coord_name =
    stroke_preserve ?path ?do_fit vp coord_name;
    if path = None then add_instruction (fun () -> Path.clear vp.path) vp

  let fill_preserve ?path ?(do_fit=true) vp coord_name =
    let path = get_path ~notransform:true vp path coord_name in
    if do_fit && coord_name = Data then fit vp (Path.extents path);
    let path = Path.copy path in
    add_instruction (fill_direct ~path vp coord_name) vp

  let fill ?path ?do_fit vp coord_name =
    fill_preserve ?path ?do_fit vp coord_name;
    if path = None then add_instruction (fun () -> Path.clear vp.path) vp
