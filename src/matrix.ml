...
struct
  ...
  let of_matrix m =
    ...
    { xx = m.Affine.xx;  yy = m.Affine.yy;
      x0 = m.Affine.x0;  y0 = m.Affine.y0 }

  let make_identity () = { xx=1.; yy=1.; x0=0.; y0=0. }
  ...
  let transform_distance m ~dx ~dy = (m.xx *. dx,  m.yy *. dy)

  let transform_point m ~x ~y = (m.xx *. x +. m.x0,  m.yy *. y +. m.y0)
  ...
  (* allow [c] to be [a] or [b]. *)
  let mul_in c b a =
    let c_xx = b.xx *. a.xx in
    let c_yy = b.yy *. a.yy in
    let c_x0 = b.xx *. a.x0 +. b.x0 in
    c.y0 <- b.yy *. a.y0 +. b.y0;
    c.xx <- c_xx;
    c.yy <- c_yy;
    c.x0 <- c_x0
