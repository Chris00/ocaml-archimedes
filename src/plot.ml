...
struct
  ...
  let unsafe_y vp ?base ~fill ~fillcolor ~style x y n =
    match style with
    | `Lines ->
  ...
  let y vp ?base ?(fill=false) ?(fillcolor=default_fillcolor)
      ?(style=`Points "O") ?(const=false) ydata =
  ...
  let unsafe_xy vp ~fill ~fillcolor ~style x y n =
    match style with
    | `Lines ->
  ...
  let xy vp ?(fill=false) ?(fillcolor=default_fillcolor)
      ?(style=`Points "O") ?(const_x=false) xdata ?(const_y=false) ydata =
