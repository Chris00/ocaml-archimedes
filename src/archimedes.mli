...
val plotxy : t -> ?axes:([>Axes.axes],[>Axes.tic]) Axes.t ->
...
module Viewport: sig
  ...
  val sub: vp -> xmin:float -> xmax:float -> ymin:float -> ymax:float -> vp
    (**Same as [new_viewport] except that the creation of the viewport
       uses another viewport. Note that the created viewport will
       depend on the viewport which made it; as a consequence, a
       changement on the coordinates in the initial viewport will
       induce a changement in the coordinates in the created one.*)

  val use: vp -> unit
    (**Makes the Archimedes handle use the coordinates stored in this viewport.*)
